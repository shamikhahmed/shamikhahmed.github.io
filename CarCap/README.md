...
Offline-first vehicle PWA — garage, service log, fuel/odometer, and a text docs wallet (meta only).

**Version:** 0.2.0 · **SW cache:** `carcap-v3` · Capricorn Systems (Cap family)

## Features (v0.2.0)

- **Garage** — make / model / year / plate (+ optional nickname)
- **Service** — log type, date, odometer, cost, notes, reminder date
- **Fuel** — liters, cost, odometer, full-tank flag; simple L/100 from ≥2 full tanks
- **Docs** — title / insurance / registration notes (text + expiry; photos later)
- **Today** — active vehicle snapshot, reminders, recent activity
- **Settings** — demo mode, JSON export/import, reset, install tips
- **First-run** — Add car vs Try demo sheet until onboarded
...
Open http://localhost:8790 — or `?demo=1` for seeded data. Pitch: `docs/pitch.html`.
...
All data stays in the browser. No accounts, no cloud sync in v0.2.0. Docs are text/meta + expiry only — no photo wallet yet.
